...
Visita el github https://github.com/espakodi para ver el código del addon completo, colaborar o usar la API.
"""
import json
import os
import re
...
def _add_to_history(url, url_type=""):
    """Anade una URL al inicio del historial (atomico)."""
    with _history_lock:
        path = _history_path()
        if not path:
            return
        # Load dentro del lock para evitar race condition
        try:
...
def _scan_videos_hybrid(url):
    ...
    En PC intenta yt-dlp primero; si no hay resultados o no está
    disponible, usa el escáner HTML como fallback universal.
    """
...
class _RemoteHandler(BaseHTTPRequestHandler):
    ...
    def _handle_scan(self):
        ...
        try:
            ...
            xbmc.log(
                "[EspaTV/remote] Error escaneando {0}: {1}".format(url[:60], exc),
                xbmc.LOGWARNING,
            )
